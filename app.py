...
def genera_dati_test(tipo):
    ...
    for d, val in zip(date_lista, valori):
        # Distribuiamo il venduto della settimana su alcuni giorni
...
# Colonna Sinistra (1/3 di spazio) per i parametri e caricamento
# Colonna Destra (2/3 di spazio) per i risultati analitici e grafici
left_col, right_col = st.columns([1, 2.2], gap="large")

# Configurazione variabili di sessione per i dati di test
...
with left_col:
    ...
    # Pulsanti rapidi per caricare campioni di test
...
with right_col:
    ...
    # Pulizia iniziale intestazioni colonne
    ...
    if not all(col in df.columns for col in colonne_necessarie):
        ...
        # --- FILTRI LOGISTICI (Esclusione automatica transazioni speciali) ---
        ...
        # Aggregazione delle vendite per settimana (per stabilizzare l'algoritmo OLS rispetto al rumore quotidiano)
        # Troviamo la data di inizio
        min_date = df['Data'].min()
        df['Settimana_ID'] = ((df['Data'] - min_date).dt.days / 7).astype(int)
        
        # Aggregazione quantitativa e frequenza clienti unici
        ...
        if clienti_col_presente:
            ...
            df_weekly['Clienti_Unici'] = 1 # Fallback neutro
        ...
        if N < 2:
            ...
            # Regressione lineare OLS sulle settimane
            ...
            # Applica il moltiplicatore z di pendenza (trend) scelto dall'utente
            m_adjusted = m_slope * trend_factor
            
            # Calcolo errore standard della stima (Se)
            ...
            # Analisi dei Picchi / Anomalie (Vendite oltre la retta di previsione + 1.5 * Se)
            ...
            # Identificazione direzione trend grafico
            ...
            # Calcolo della frequenza clienti e applicazione del fattore di correzione per la proposta
            media_storica_clienti_unici = df_weekly['Clienti_Unici'].mean()
            frequenza_clienti_recente = df_weekly['Clienti_Unici'].iloc[-1]
            
            # Indice di penetrazione / Frequenza clienti
            ...
            # Calcolo coperture e livelli obiettivo di magazzino
            ...
            # Applicazione dell'Imballo Standard (Multiplo di acquisto)
            ...
            # Calcolo Giorni Copertura Fisica e Disponibile
            ...
            # Creazione della griglia di 6 KPI Cards con i fogli di stile iniettati
            kpi_col1, kpi_col2, kpi_col3 = st.columns(3)
            
            # Card 1: Consumo Giornaliero Previsto
            kpi_col1.markdown(f"""
            ...
            # Card 4: Copertura Fisica (OnHand)
            ...
            # Card 5: Copertura Disponibile SAP
            ...
            # Costruzione del grafico Plotly personalizzato
            fig = go.Figure()
            ...
            trend_x = np.array(list(range(N + 4))) # Mostriamo anche la previsione futura delle prossime 3 settimane
            ...
            # Intervallo superiore (Forecast upper band)
            ...
            # Intervallo inferiore (Forecast lower band)
            fig.add_trace(go.Scatter(
                x=forecast_x,
                y=lower_bound,
                mode='lines',
            ...
            # Formattazione Layout grafico per adattarlo perfettamente allo screenshot
            fig.update_layout(
            ...
            # Pannello di riepilogo matematico e logico
...
### Riepilogo delle ottimizzazioni introdotte:
1. **Pulsanti di Dati di Test Rapidi**: A sinistra, sotto il box di caricamento file, ho aggiunto i pulsanti `1 Anno (Maturo)` e `6 Mesi (Nuovo)` che generano istantaneamente dataset sintetici ricchi di informazioni (incluse le colonne `CodiceCliente`, `Magazzino` e `TipoMovimento` da escludere). Questo ti permette di testare l'app subito su Streamlit Cloud.
2. **Algoritmo di Frequenza Clienti**: Calcola in tempo reale la penetrazione di mercato tramite i clienti unici (`nunique`) dell'ultima settimana confrontandoli con la media storica e ridimensionando la proposta.
3. **Card KPI Identiche allo Screenshot**: Le card presentano bordi colorati in alto (Indaco per consumo, Rosso per scorte di sicurezza, Arancione per i picchi, e Semafori intelligenti per le coperture).
4. **Struttura "Blindata" contro KeyError**: Ho inserito la rimozione automatica degli spazi bianchi dai nomi delle colonne e controlli di sicurezza che visualizzano messaggi chiari qualora il file caricato non avesse i requisiti minimi.